...
function showNotification(message, isError = false) {
  ...
  if (notify) {
    ...
    notify.style.backgroundColor = isError
      ? "rgba(255, 0, 0, 0.1)"
      : "rgba(0, 255, 0, 0.1)";
...
async function deleteTickets() {
  ...
  try {
    ...
    if (isFiltered) {
      ...
      showNotification(
        `Tickets erfolgreich gelöscht (${ticketsToDelete.length} Tickets für Lehrjahr ${lj})`
      );
    } else {
      ...
      if (!resp.ok) throw new Error(`Server ${resp.status}`);
      showNotification(
        "Tickets erfolgreich gelöscht (alle Tickets aus allen Lehrjahren)"
      );
    }
    ...
    if (deleteSelectedBtn) {
      deleteSelectedBtn.addEventListener("click", async () => {
        if (selectedTicketIds.size === 0) {
          showNotification(
            "Bitte wählen Sie zuerst ein oder mehrere Tickets aus.",
            true
          );
          return;
        }
        ...
        try {
          ...
          showNotification(
            `Ticket${
              count > 1 ? "s" : ""
            } erfolgreich gelöscht (${count} ${ticketText})`
          );
          selectedTicketIds.clear();
          const lj = document.getElementById("lehrjahr")?.value || "";
