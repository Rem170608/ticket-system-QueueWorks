    ...
    if (!data.adminExists) {
      ...
      setTimeout(() => {
        ...
        sessionStorage.setItem(
          "setupMessage",
          "Admin existiert nicht. Weiterleitung..."
        );
      }, 2000);
      return;
    }
    ...
    if (errorMessage) {
      if (error.message.includes("Failed to fetch")) {
        errorMessage.textContent =
          "Server nicht erreichbar. Bitte stellen Sie sicher, dass der Server läuft und auf Port 3000 erreichbar ist.";
      } else {
...
document
  ...
  ?.addEventListener("click", async function () {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error(
          "Server hat keine JSON-Antwort zurückgegeben. Läuft der Server?"
        );
      }
      ...
      if (response.ok && data.token) {
        ...
        errorMsg.textContent =
          data.message || "Ungültiger Username oder Passwort.";
        errorMsg.style.display = "block";
